export const questionBank = [
  ...
  },
  {
    ...
    keywords: ['测试范围', '正常流程', '异常场景', '边界条件', '回归策略'],
    referenceAnswer: '回答应覆盖测试分层和测试重点。先明确需求与接口契约，再设计正常流程、异常参数、边界值、幂等与重复提交、超时重试、权限校验等场景；接口测试要关注入参、出参、状态码、数据库落库和下游调用；回归上要区分冒烟、核心链路回归和发布前全量回归，并说明如何把关键场景沉淀为自动化。',
    ...
    scoringRubric: {
      mustHave: ['测试范围', '异常场景', '边界条件', '回归策略'],
      goodToHave: ['接口契约', '幂等', '下游依赖', '自动化沉淀'],
      redFlags: ['只说点点页面', '没有接口和数据校验', '没有异常与边界意识']
    }
  },
  {
    id: 'qa_002',
    ...
    keywords: ['现状问题', '自动化方案', '质量门禁', '推进落地', '结果指标'],
    referenceAnswer: '项目题要说明业务背景、原有质量问题、你推动的改进方案、与研发产品协作的方式以及最终结果。高质量回答通常会提到覆盖率、缺陷率、回归耗时、发布成功率或漏测率等指标，并能说明为什么选择当前自动化分层或质量门禁方案。',
    ...
    scoringRubric: {
      mustHave: ['现状问题', '自动化方案', '推进落地', '结果指标'],
      goodToHave: ['质量门禁', '协作阻力', '稳定性治理', '覆盖率或回归耗时'],
      redFlags: ['只讲工具不讲问题', '没有个人主导动作', '没有结果证据']
    }
  },
  {
    id: 'ops_001',
    ...
    keywords: ['CPU', '负载', '进程线程', '系统日志', '业务日志'],
    referenceAnswer: '排查应先确认现象和影响范围，再从系统层逐步收敛。通常先看 CPU、load、内存、磁盘和网络基础指标，定位是单进程还是整体资源紧张；再看高占用进程和线程、系统日志、最近发布和定时任务；最后结合业务日志、网关日志和依赖服务状态判断是应用热点、死循环、GC、I/O 等待还是网络异常。',
    ...
    scoringRubric: {
      mustHave: ['CPU', '负载', '进程线程', '系统日志', '业务日志'],
      goodToHave: ['iowait', '网络指标', '最近变更', '止血和复盘'],
      redFlags: ['上来就重启', '没有排查顺序', '只看系统不看业务链路']
    }
  },
  {
    id: 'ops_002',
    ...
    keywords: ['告警发现', '排查链路', '止血动作', '根因分析', '后续治理'],
    referenceAnswer: '好的回答要有完整故障闭环：如何发现、影响范围、排查顺序、临时止血方案、根因确认以及事后治理。面试官通常会看你是不是有明确时间线、能否说明为什么先做哪些动作，以及是否把问题沉淀为监控、容量、预案或自动化改进。',
  ...
  },
  {
    ...
    keywords: ['CI/CD', '构建', '测试', '发布', '回滚', '权限控制'],
    referenceAnswer: '一个基本流水线通常包括代码检查、构建打包、单元测试、接口或冒烟测试、制品管理、环境发布、发布审批和回滚方案。回答时要说明不同环境的门禁、如何确保产物一致性、谁有权限执行发布，以及失败时如何快速回滚。',
    ...
    scoringRubric: {
      mustHave: ['CI/CD', '测试', '发布', '回滚', '权限控制'],
      goodToHave: ['制品管理', '灰度发布', '审批', '环境一致性'],
      redFlags: ['只有脚本没有门禁', '没有回滚方案', '权限边界不清']
    }
  },
  {
    id: 'devops_002',
    ...
    keywords: ['日志', '指标', '链路追踪', '告警', '降噪', 'SLO'],
    referenceAnswer: '可观测性平台设计要覆盖三大支柱：指标、日志和链路追踪，并结合告警和值班流程。回答时要说明采集方式、存储与查询、统一关联维度、告警阈值与抑制策略，以及如何让业务团队真正用起来。较好的答案会提到 SLI/SLO、告警分级、去重和根因定位效率。',
    ...
    scoringRubric: {
      mustHave: ['日志', '指标', '链路追踪', '告警'],
      goodToHave: ['SLO', '降噪', '统一字段', '权限和成本'],
      redFlags: ['只堆工具名', '没有使用场景', '忽略告警治理和关联查询']
    }
  },
  {
    id: 'data_001',
    ...
    keywords: ['ODS', 'DWD', 'DWS', 'ADS', '分层建模', '口径一致'],
    referenceAnswer: '数仓分层的核心是解耦原始数据接入、明细加工、主题汇总和应用展示，降低重复开发并保证口径一致。ODS 保存贴近源系统的原始数据，DWD 做清洗和业务明细建模，DWS 做主题聚合和公共指标沉淀，ADS 面向具体报表或应用场景输出结果。',
    ...
    scoringRubric: {
      mustHave: ['ODS', 'DWD', 'DWS', 'ADS'],
      goodToHave: ['口径一致', '复用', '主题域', '治理成本'],
      redFlags: ['只背缩写', '讲不清每层职责', '忽略口径治理']
    }
  },
  {
    id: 'data_002',
    ...
    keywords: ['数据来源', '调度依赖', '数据质量', '指标口径', '业务价值'],
    referenceAnswer: '项目回答要说明数据从哪里来、如何加工、调度如何编排、质量如何校验、最终服务了什么业务。较好的回答会提到离线或实时链路、血缘依赖、补数方案、异常告警、指标口径统一，以及具体业务成效。',
    ...
    scoringRubric: {
      mustHave: ['数据来源', '调度依赖', '数据质量', '业务价值'],
      goodToHave: ['补数', '口径统一', '血缘', '异常告警'],
      redFlags: ['只讲开发脚本', '没有质量控制', '没有业务落地结果']
    }
  },
  {
    id: 'ai_001',
    ...
    keywords: ['离线评估', '线上评估', '准确率', '召回率', 'AUC', '业务指标'],
    referenceAnswer: '模型评估要结合任务类型和业务目标。分类任务常看准确率、精确率、召回率、F1、AUC；推荐任务还会关注点击率、转化率、NDCG 等。离线评估依赖历史数据和标注集，适合快速比较模型；线上评估通过 A/B 实验验证真实业务效果，还要关注延迟、稳定性和成本。',
    ...
    scoringRubric: {
      mustHave: ['离线评估', '线上评估', '准确率或精确率召回率', '业务指标'],
      goodToHave: ['AUC', 'A/B 实验', '延迟和成本', '任务差异'],
      redFlags: ['只会背指标名', '讲不清离线和线上差异', '忽略业务目标']
    }
  },
  {
    id: 'ai_002',
    ...
    keywords: ['数据来源', '效果评估', '上线方式', '推理成本', '业务取舍'],
    referenceAnswer: '项目题需要覆盖问题背景、数据来源与清洗、模型或方案选择、离线和线上评估方式、部署链路以及效果与成本的平衡。优秀回答会说明为什么选择当前模型、如何处理数据质量、如何监控效果漂移，以及上线后带来了什么业务改进。',
    ...
    scoringRubric: {
      mustHave: ['数据来源', '效果评估', '上线方式', '成本取舍'],
      goodToHave: ['灰度发布', '漂移监控', '缓存或分层模型', '业务结果'],
      redFlags: ['只讲模型名', '没有评估闭环', '忽略成本和稳定性']
    }
  },
  {
    id: 'security_001',
    ...
    keywords: ['XSS', 'CSRF', 'SQL 注入', '输入校验', '输出编码', '鉴权'],
    referenceAnswer: 'XSS 是恶意脚本被注入并在受害者浏览器执行，常见防护有输出编码、CSP、输入过滤；CSRF 是借用用户已登录状态发起未授权请求，常见防护有 CSRF token、SameSite Cookie、关键操作二次校验；SQL 注入是拼接 SQL 导致恶意语句执行，防护重点是参数化查询、最小权限和输入校验。',
    ...
    scoringRubric: {
      mustHave: ['XSS', 'CSRF', 'SQL 注入', '防护手段'],
      goodToHave: ['输出编码', 'CSP', 'SameSite', '参数化查询'],
      redFlags: ['概念混淆', '只会背名词', '防护方案停留在黑名单']
    }
  },
  {
    id: 'security_002',
    ...
    keywords: ['风险评估', '修复方案', '验证方法', '防复发', '治理闭环'],
    referenceAnswer: '项目题应说明漏洞如何发现、影响范围、风险等级判断、修复思路、验证过程和后续治理。好的回答会强调不是单个漏洞修掉就结束，而是要补规范、扫描、权限或架构层面的长期改进，避免同类问题反复出现。',
    ...
    scoringRubric: {
      mustHave: ['风险评估', '修复方案', '验证方法', '防复发'],
      goodToHave: ['影响范围', '统一治理', '扫描规则', '权限模型'],
      redFlags: ['只修表面问题', '没有验证闭环', '没有防复发措施']
    }
  },
  {
    id: 'architect_001',
    ...
    keywords: ['演进路径', '高可用', '拆分边界', '风险控制', '技术债'],
    referenceAnswer: '架构演进题重点不是堆技术名词，而是说明为什么演进、先解决什么瓶颈、如何控制风险。通常要结合业务增长、团队能力和历史系统现状，设计分阶段方案，例如先补监控与治理，再抽离高风险模块、引入缓存和异步解耦，最后再考虑更细粒度拆分。回答中应体现技术债、迁移成本和回滚策略。',
    ...
    scoringRubric: {
      mustHave: ['演进路径', '拆分边界', '风险控制', '高可用'],
      goodToHave: ['观测治理', '回滚方案', '收益指标', '技术债'],
      redFlags: ['直接全量重构', '没有阶段性目标', '不考虑团队和迁移成本']
    }
  },
  {
    id: 'architect_002',
    ...
    keywords: ['背景', '决策依据', '推进阻力', '治理动作', '结果'],
    referenceAnswer: '这类项目题会看你是否具备跨团队技术判断和推进能力。回答应说明为什么需要治理、你基于什么数据或风险做决策、如何处理不同意见，以及最终结果是稳定性、效率还是成本上的改善。',
