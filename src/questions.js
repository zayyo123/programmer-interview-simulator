export const questionBank = [
  {
    ...
    "referenceAnswer": "（示例）订单履约系统：业务目标是提升下单后库存确认和发货链路稳定性。个人职责包括订单状态流转、库存扣减一致性和消息重试机制；技术栈 Spring Boot、MySQL、Redis、RabbitMQ。关键问题：订单创建成功但库存消息偶发失败，根因是消息投递无可靠保障。方案：本地消息表记录待发送事件、消费者幂等处理、定时任务补偿失败消息，保证最终一致性。结果：异常订单人工处理比例明显下降。完整作答须覆盖背景、个人贡献、技术选型理由、量化指标和复盘，避免只讲团队或堆砌名词。",
  ...
  },
  {
    ...
    "referenceAnswer": "从几个方面看。首先 Redis 主要基于内存读写，延迟很低。其次它用了 I/O 多路复用，可以用较少线程处理大量连接。第三，Redis 的核心命令长期采用单线程执行，避免了多线程锁竞争和频繁上下文切换。再加上 hash、skiplist、quicklist 等数据结构针对典型场景做了优化，所以它快不是单纯因为内存，而是内存、网络模型、线程模型和数据结构共同作用。Redis 6.0+ 可用 IO 线程处理网络读写，但命令执行仍在主线程，避免多线程锁竞争；RESP 协议简单，解析开销低。Redis 6.0+ IO 线程处理网络读写，命令执行仍在主线程；hash/skiplist/quicklist 针对典型操作优化，RESP 协议解析开销低。",
  ...
  },
  {
    ...
    "referenceAnswer": "索引的核心价值是减少扫描量。以 InnoDB 为例，B+ 树索引是有序的，可以通过较少的层级定位到目标范围，而不是把整张表扫一遍。主键索引的叶子节点就是行数据，二级索引叶子节点存的是主键，所以通过二级索引查非索引字段可能需要回表。如果查询字段都在索引里，就可以走覆盖索引，进一步减少 I/O。B+ 树非叶子节点仅存键和指针，叶子节点通过链表相连，适合范围扫描和排序；哈希索引等值查询快，但不支持范围查询、排序和最左前缀匹配。哈希索引等值快但不支持范围查询、排序和最左前缀。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先将隔离级别和实现机制分开讲。事务隔离级别定义的是并发读写下允许看到什么现象；InnoDB 的 MVCC 是实现一致性读的重要手段，核心是版本链和 Read View。读已提交下一条语句通常会生成新的 Read View，所以可能看到其他事务已提交的新数据；可重复读下事务内快照读通常复用同一个 Read View，所以多次读结果更稳定。幻读本质是范围查询里出现了之前不存在的新行，InnoDB 对普通快照读用 MVCC 避免读到不该看到的版本，对当前读或加锁读会用 next-key lock 锁住索引范围，防止其他事务插入命中的记录。线上死锁首先收集死锁日志和 SHOW ENGINE INNODB STATUS，确认两个事务分别持有什么锁、等待什么锁，再看 SQL 是否走了合适索引、是否访问顺序不一致、是否范围锁过大，优化方向包括统一加锁顺序、补充索引缩小锁范围、拆小事务和减少事务内慢操作。注意：InnoDB 可重复读下快照读与当前读行为不同；普通 SELECT 快照读主要依赖 MVCC，SELECT FOR UPDATE 等当前读会加 next-key lock。InnoDB 可重复读下：普通快照读靠 MVCC+Read View；当前读（SELECT FOR UPDATE 等）会加 next-key lock 防范围插入。死锁排查用 SHOW ENGINE INNODB STATUS，看持锁/等待关系，优化统一加锁顺序和索引命中。",
  ...
  },
  {
    ...
    "referenceAnswer": "按“先定位、再解释计划、最后改方案”的顺序回答。第一步先确认这条 SQL 是偶发慢还是持续慢，慢在数据库执行还是网络/应用等待，可以从慢查询日志、APM 调用链、数据库监控里拿到 SQL、耗时、扫描行数和执行频率。第二步用 Explain 看执行计划，重点看 type 是否退化为 ALL，key 是否用了预期索引，rows 是否明显过大，Extra 里有没有 Using filesort、Using temporary、Using where。第三步结合 SQL 条件和业务语义优化：如果 where、order by、group by 没有合适索引，就考虑联合索引并遵守最左前缀；如果函数、隐式类型转换或前导模糊匹配导致索引失效，就改写条件；如果深分页慢，可以改成基于游标或延迟关联；如果统计类查询频繁，可以做汇总表或缓存。最后还要压测或灰度验证，避免新索引带来写入成本和存储成本。Explain 重点看 type 是否 ALL、key 是否命中、rows 是否过大、Extra 是否有 Using filesort/Using temporary。深分页可改游标分页或延迟关联；新索引要评估写入成本和存储成本。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先讲连接建立：第一次客户端发 SYN，表示想建立连接并带上初始序列号；第二次服务端返回 SYN+ACK，说明服务端收到了客户端请求，也把自己的初始序列号发给客户端；第三次客户端发 ACK，服务端收到后双方都确认对方的发送和接收能力正常。之所以不是两次，是为了避免旧的 SYN 报文在网络里滞留后被服务端误认为新连接，也无法让服务端确认客户端已经收到自己的响应。关闭连接通常需要四次，是因为 TCP 是全双工，一端不发了不代表另一端也发完了，所以 FIN 和 ACK 往往分开。排查接口偶发超时时，把网络层拆成域名解析、建连、传输和网关四段：看 DNS 耗时、TCP connect 耗时、重传率、连接池耗尽、SYN backlog、带宽打满、跨机房 RTT、负载均衡 5xx/超时，以及客户端是否有超时重试放大流量。关闭连接时主动方会进入 TIME_WAIT，用于等待迟到的报文失效，避免影响后续复用连接；排查超时要区分 DNS、建连、TLS、传输和服务端处理阶段。TIME_WAIT 在主动关闭方等待迟到报文失效；排查超时要分段看 DNS、TCP connect、重传率、连接池、SYN backlog、带宽、跨机房 RTT。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先将“超时”拆成具体阶段。第一段是 DNS，看解析耗时、缓存命中和是否有异常域名解析；第二段是建连，看 TCP connect 耗时、重传、跨机房 RTT；第三段是 HTTPS 特有的 TLS 握手，看证书校验、会话复用和握手耗时；第四段是连接池，看是否连接池耗尽、空闲连接失效、keep-alive 配置不一致；第五段看网关和负载均衡，比如是否某个 upstream 慢、排队、限流或 5xx 增多；最后才看服务端应用和下游依赖。HTTPS 的额外成本主要来自 TLS 握手和加解密，风险点包括证书过期、SNI/域名不匹配、协议版本或加密套件兼容问题。真实落地时用客户端分段耗时、网关访问日志、APM trace 和连接池监控一起对齐时间线，同时关注重试是否把偶发慢放大成流量峰值。TLS 会话复用可降低握手开销；注意证书过期、SNI 不匹配、keep-alive 与连接池配置不一致导致的建连慢。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先将状态码和调用策略关联起来。2xx 是成功；301/302/307/308 要关注方法是否保持和重定向循环；400、401、403、404 多数是请求或权限问题，通常不应该重试；408、429、502、503、504 这类更可能是临时性问题，可以在有限次数内重试。重试策略一定要和超时、限流、熔断一起设计：每次请求要有总超时和单次超时，重试次数有限，使用指数退避和随机抖动，避免所有客户端在同一时间再次打到服务端。对于写接口，重试前必须设计幂等，比如客户端生成 idempotency key，服务端用唯一索引或幂等表记录请求结果；订单、支付这类链路还要结合业务单号、状态机和去重消费，保证重复请求返回同一结果而不是重复扣款或重复创建。最后还要有监控，关注重试率、429/5xx、幂等冲突数和下游错误，防止重试把局部故障放大成全链路故障。400/401/403/404 通常不可重试；408/429/502/503/504 可有限重试。写接口幂等键+唯一索引+状态机；指数退避+随机抖动+熔断防重试风暴。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先将概念讲清楚：进程更像资源容器，有独立地址空间，进程间通信成本相对更高；线程运行在进程里，共享堆、文件描述符等资源，但每个线程有自己的栈、寄存器上下文和调度状态。线程更轻量，但不是越多越好，线程过多会造成上下文切换增加，锁竞争也可能把 CPU 消耗在等待或自旋上。排查 CPU 飙高时，首先看监控确认是某台机器、某个进程还是某个时间段整体升高，再用 top、pidstat 或类似工具定位到具体进程和线程；如果是 Java 服务，会把高 CPU 线程 id 转换成十六进制去 jstack 里找对应线程栈；如果是 Go 或 Python，也会结合 pprof、采样或火焰图看热点函数。最后再区分是业务死循环、热点计算、频繁 GC、锁竞争、系统调用过多，还是内核态网络/磁盘问题。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先说它解决的问题：高并发连接下，如果每个连接都阻塞等待 I/O，会产生大量线程和上下文切换。I/O 多路复用让线程先把多个 fd 注册给内核，等这些 fd 中有读写事件就绪时再回来处理。select 的问题是 fd 集合大小有限，并且每次调用都要传入集合，返回后还要遍历；poll 取消了固定大小限制，但仍然需要遍历；epoll 把关注的 fd 放在内核里维护，事件就绪后放入就绪队列，应用拿到的是活跃事件，所以在大量长连接场景下效率更好。Reactor 可以理解为“事件循环 + 事件分发”：主线程或 event loop 负责监听连接和 I/O 就绪，业务 handler 处理读写和协议解析，耗时任务再交给 worker 池，避免阻塞事件循环。Redis、Netty、Nginx 这类系统的高并发能力，都和事件驱动、非阻塞 I/O、合理的线程模型有关。epoll 用就绪队列返回活跃 fd，适合大量长连接；Reactor 耗时业务应放 worker 池，避免阻塞 event loop。",
  ...
  },
  {
    ...
    "referenceAnswer": "HashMap 可以理解为一个桶数组。put 时先根据 key 的 hash 定位桶位置，如果没有冲突就直接放入；如果冲突，就在桶里用链表或红黑树组织节点。Java 8 以后，当链表过长并且数组容量达到条件，会把链表转成红黑树，避免极端情况下查询退化得太厉害。扩容时容量翻倍，元素会根据新的容量重新分布，所以频繁扩容会有成本。put 时先计算 hash 定位桶；链表长度超阈值（默认 8）且数组容量≥64 时转红黑树；容量保持 2 的幂便于位运算取模；负载因子默认 0.75 触发扩容并 rehash。put：hash 定位桶→冲突拉链表/红黑树（链表≥8 且容量≥64 树化）；负载因子 0.75 触发扩容 rehash。HashMap 非线程安全。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先讲流程，而不是只背参数。线程池收到任务后，先看当前线程数是否小于 corePoolSize，是的话直接创建核心线程执行；否则尝试放入 workQueue；如果队列也满了，再看是否能扩到 maximumPoolSize；如果仍然不能接收，就走拒绝策略。拒绝策略里 AbortPolicy 会抛异常，CallerRunsPolicy 会让提交任务的线程自己执行，从而形成一定反压，DiscardPolicy 和 DiscardOldestPolicy 则会丢任务，业务上要非常谨慎。线上打满时，首先看是线程数打满还是队列堆积，结合任务耗时分布、下游接口耗时、线程栈和拒绝次数判断是下游慢、任务过重、流量突增还是线程池混用。治理上不会只盲目加线程，而是做业务隔离、核心链路限流、任务超时、队列长度告警、慢任务拆分；对于关键任务，还要设计补偿或重试，避免被拒绝后静默丢失。无界队列会使 maximumPoolSize 失效；CallerRunsPolicy 形成反压但可能阻塞提交线程；核心与低优先级任务应分池隔离并监控队列长度。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先按问题类型区分。需要互斥保护临界区时可以用 synchronized 或 ReentrantLock；如果只是一个状态标记，需要让其他线程及时看见，并避免重排序影响，可以用 volatile，但像 i++ 这种复合操作不能只靠 volatile。ReentrantLock 比 synchronized 更灵活，比如 tryLock、lockInterruptibly、公平锁和多个 Condition，但也要求 finally 里手动 unlock。ConcurrentHashMap 解决的是并发读写 Map 的线程安全和性能问题，不能简单用 HashMap 加外层大锁替代。线上怀疑锁竞争时，首先看接口耗时、线程状态和阻塞数量，再抓线程栈或用监控看 blocked/waiting 线程集中在哪个锁；如果发现大量线程卡在同一段同步代码，会检查临界区是否包含慢 I/O、远程调用、大循环或热点资源。治理上通常会缩小锁粒度、减少锁内耗时、拆分热点 key、用无锁或读写分离结构，必要时做限流和降级。volatile 不保证 i++ 原子性；ReentrantLock 须在 finally unlock；ConcurrentHashMap 桶级锁/CAS，不能简单 HashMap+大锁替代。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先区分是网络慢、服务端慢、资源太大还是渲染慢。工具上看 Lighthouse 指标和 Performance 主线程，再看 Network 的瀑布图、资源大小和 TTFB。如果瓶颈在资源体积，会做代码分割、移除无用依赖、压缩图片和懒加载；如果是渲染阻塞，会处理关键 CSS、延迟非关键脚本；如果是接口慢，会考虑缓存、接口聚合或 SSR/SSG。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先拆成写入和读取。写入是用户提交长链接，服务生成唯一短码并保存映射；读取是用户访问短码，系统查到长链接后重定向。短码生成可以用发号器或分布式 ID 再做 Base62 编码，避免冲突。读链路通常压力更大，所以会把短码映射放入 Redis 缓存，DB 做最终存储。还要考虑过期时间、访问统计、恶意链接检测、限流，以及服务和存储的高可用。短码可用发号器+Base62；301 永久跳转利于 SEO，302 便于变更和统计；碰撞用唯一约束或重试生成；读链路缓存+DB 持久化，考虑过期和恶意链接检测。",
  ...
  },
  {
    ...
    "referenceAnswer": "用哈希表把查找另一个数的成本降到 O(1)。遍历数组时，当前值是 x，需要找的是 target - x。如果这个差值已经在 map 里，说明之前出现过对应数字，直接返回之前下标和当前下标；如果没有，就把当前数字和下标存进去。这样只需要遍历一次，时间复杂度 O(n)，额外空间 O(n)。",
  ...
  },
  {
    ...
    "referenceAnswer": "（示例）后台工单系统。项目目标是把原来线下流转的售后流程搬到线上，减少人工统计。个人职责包括负责工单列表、状态流转和通知模块，技术栈包括 Java、Spring Boot、MySQL 和 Vue。关键实现包括一个超时提醒功能，先梳理了不同状态下的超时规则，再通过定时任务扫描待处理工单并触发站内信提醒，最终让客服漏处理工单的情况明显减少。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先确认问题范围，是整体变慢还是部分实例、部分命令变慢，然后结合监控看延迟从什么时候开始抖动。第一步排命令层，查看慢查询和大 key，确认是不是某些命令本身耗时过高。第二步看网络和连接数，包括连接暴涨、网卡带宽、跨机房访问和客户端超时重试。第三步看内存状态，比如是否接近 maxmemory、是否发生频繁淘汰、内存碎片率是否异常。第四步看持久化，如果刚好在做 AOF rewrite 或 RDB fork，也可能带来抖动。最后再结合 CPU、QPS 和实例日志收敛到根因。命令层：SLOWLOG、INFO commandstats、redis-cli --bigkeys、MEMORY USAGE。网络层：连接数、带宽、跨机房 RTT、客户端重试放大。内存层：maxmemory、淘汰策略、mem_fragmentation_ratio。持久化：AOF rewrite、RDB fork 的 CPU/磁盘抖动。结合 latency doctor 和时间线收敛，避免直接重启。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先说明这不是绝对强一致方案，而是在性能和一致性之间做取舍。多数业务用 cache aside：读时先读 Redis，未命中再读 DB 并写缓存；写时先提交数据库，再删除缓存。优先删除而不是更新缓存，是因为缓存可能是聚合结构，直接更新容易漏字段或漏场景。真正落地时要处理几个边界：第一，删除缓存失败要有重试或消息补偿，比如写入 MQ、订阅 binlog，或者后台任务扫描补偿；第二，缓存必须有 TTL 兜底，防止脏数据无限存在；第三，要注意并发读写，写库后删缓存期间，如果旧查询回填了缓存，可以通过延迟双删、版本号或热点 key 互斥重建降低概率；第四，如果业务要求强一致，比如余额扣减，就不能只依赖缓存一致性方案，应该以数据库事务或专门的一致性链路为准。Cache Aside：写路径先提交 DB 再删缓存；删缓存失败要 MQ/binlog/重试补偿；必须有 TTL 兜底；并发读写注意延迟双删或版本号；余额等强一致场景不能只靠缓存方案。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先确认影响范围，是全量白屏还是部分路由、部分浏览器白屏，再结合埋点和前端监控看是不是某个版本发布后开始出现。止血上优先考虑回滚静态资源版本，或者对问题路由做降级兜底，避免继续扩大影响。定位时看 runtime error、资源加载失败、接口超时和 hydration 异常，必要时在入口加错误边界和关键日志。事后治理会补齐 source map 上报、灰度发布、核心页面可用性监控，以及对配置或动态数据做更严格的容错。区分 runtime error、资源 404、接口超时、SSR hydration 失败；止血优先回滚静态资源；治理补 source map、灰度、可用性监控。",
  ...
  },
  {
    ...
    "referenceAnswer": "按入口、核心交易链路和兜底治理来拆。入口层先做活动预热、CDN 和网关限流，必要时配合验证码或人机校验做防刷。库存层优先在 Redis 或内存里做原子预扣，避免所有请求直接打数据库，再通过消息队列异步创建订单。真正落库时要保证幂等，比如用用户加商品维度的唯一约束防止重复下单，同时要有超时取消和库存回补机制。最后还要关注监控告警、热点隔离、降级开关和故障演练，不然系统即使能扛住流量，也未必能稳定成交。Redis 预扣成功但落库失败要有超时取消和库存回补；热点商品做隔离和本地缓存；防刷结合限流+验证码+风控规则。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先从使用场景讲。普通列表会一次性把所有结果放进内存，适合数据量可控、需要反复随机访问的场景；生成器更适合大批量数据、分页拉取或流式处理，因为它基于 yield 按需产出，每次 next 才继续往下执行。它本质上实现了迭代器协议，所以 for 循环可以直接消费。价值主要在于节省内存、让处理链路更流式，但代价是通常只能顺序遍历一次，不适合频繁回看历史结果。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先确认瓶颈到底在 CPU、I/O 还是外部依赖，不会一上来就把问题归因到 GIL。先看监控和 profiling，找到热点函数、请求类型和是否单机普遍打满。如果是 I/O 密集但线程吞吐仍低，要看是不是有锁竞争、连接池或下游阻塞；如果是纯 Python CPU 密集逻辑，多线程通常会被 GIL 限制，这时更适合多进程、任务队列，或者把热点逻辑下沉到 C 扩展、NumPy 这类能释放 GIL 的实现。最后再判断是不是架构层的问题，比如单个进程承担了过多同步计算，应该拆成异步链路或独立计算服务。cProfile/py-spy 找热点；CPU 密集受 GIL 限制用多进程/C 扩展；I/O 密集查连接池和下游阻塞，勿一概归因 GIL。",
  ...
  },
  {
    ...
    "referenceAnswer": "goroutine 适合高并发，核心在于它不是直接映射成一个系统线程，而是由 Go runtime 做 M:N 调度。相比线程，goroutine 初始栈更小，而且能按需扩缩，所以单机可以承载更多并发任务。调度器会把 goroutine 分配到多个内核线程上运行，减少线程创建和切换成本。再加上 channel、context 这些机制，能更清晰地组织并发协作。当然它也不是没有代价，如果 goroutine 泄漏、阻塞点太多或者共享内存同步混乱，照样会把服务拖慢。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先确认 goroutine 上涨是瞬时流量带来的正常波动，还是请求结束后也回不去。先看 runtime 和业务监控，比如 goroutine 数、延迟、QPS、错误率，再抓 goroutine profile 和 block profile，看大量协程卡在什么位置，是 channel 等待、锁竞争、网络调用还是定时任务没退出。如果怀疑泄漏，重点看有没有消费端退出但生产端还在写 channel、没有 context cancel、ticker 没 stop、重试协程无上限这些常见问题。定位到热点后，再决定是修退出逻辑、缩小临界区还是把阻塞操作改成异步或限流。pprof goroutine/block profile；常见泄漏：channel 阻塞无消费者、未 context cancel、ticker 未 stop、无界重试协程。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先讲取舍：如果核心是传递任务或事件，比如 worker 从队列取任务，channel 更自然；如果多个 goroutine 要维护同一份 map、计数器或缓存状态，用 mutex 更直接，避免为了共享状态强行绕 channel。context 不是用来传业务参数的容器，主要负责取消、超时和跨 goroutine 传播生命周期。channel 的关闭原则通常是谁发送谁关闭，多个发送方时要特别谨慎，可以用额外的 done channel 或 context 控制退出。线上如果 goroutine 数持续上涨，首先看 pprof goroutine，确认大量 goroutine 卡在 send、recv、select、锁等待还是网络调用；再结合 block profile 和 mutex profile 看阻塞位置。如果是 channel 阻塞，要检查生产消费是否不对称、buffer 是否太小、退出时是否没有通知；如果是锁竞争，要看临界区里是否有慢调用、大循环或热点资源。最终治理一般是补取消路径、限制 goroutine 数、拆分热点锁、缩小临界区和增加阻塞/队列/泄漏监控。channel 谁发送谁关闭；context 负责取消传播；mutex 保护共享 map/计数器，临界区避免慢 I/O。",
  ...
  },
  {
    ...
    "referenceAnswer": "把系统拆成任务定义层、调度控制层、执行器层和观测治理层。任务定义里至少包含任务类型、触发方式（cron、延时、事件）、超时、重试策略和路由信息。调度层负责扫描到期任务并分发到队列或执行器，为了避免重复触发，需要用任务实例 ID 配合状态机或分布式锁做幂等保障。执行器拉取任务后先记录开始状态，失败按退避策略重试，超过阈值进入死信或人工处理队列。观测层要补齐执行耗时、成功率、重试次数、积压深度、失败原因分布和告警，这样平台才真的能支持内部生产场景。调度幂等：任务实例 ID+状态机/分布式锁；执行器记录开始/结束状态；观测执行耗时、成功率、积压深度。",
  ...
  },
  {
    ...
    "referenceAnswer": "不应一上来就调参数，而是先确认现象。先看监控和 GC 日志，判断是突然有对象分配暴增、老年代一直降不下来，还是某次版本上线后停顿时间明显变差。如果是分配率过高，会继续看是否有大量短命对象、重复缓冲或序列化开销；如果怀疑是对象滞留，就结合 heap dump 看缓存是否失控、监听器未释放、ThreadLocal 或大集合持有过多对象。只有在日志和堆数据支持的前提下，我才会去判断新生代、老年代大小或收集器选型是否不合适。最后再用停顿时间、GC 频率和请求 SLA 验证优化是否生效。",
  ...
  },
  {
    ...
    "referenceAnswer": "（示例） Java 订单履约链路治理项目，用户支付成功后要更新订单状态、扣减库存并触发履约通知。个人职责包括负责订单服务里的状态流转和异步编排。设计上不应将所有动作都塞进一个长事务里，而是把“本地事务落订单事件”和“异步消费后续动作”拆开。同步链路里只做订单状态校验、幂等检查和事件落库，保证用户请求尽快返回；后续库存和通知通过线程池隔离的异步消费者处理。这样做的原因是下游依赖耗时波动大，如果放在主事务里会把 RT 和失败面一起放大。为了避免异步任务堆积，我把核心履约和低优先级通知分开线程池，并配了拒绝策略和降级开关。失败后通过补偿任务按退避策略重试，同时用状态机限制非法流转。上线后高峰期超时和人工补单量都明显下降。",
  ...
  },
  {
    ...
    "referenceAnswer": "（示例）给运营和客服使用的中后台，里面有订单查询、审核流、活动配置和权限管理。主要职责包括前端架构和核心交互模块。状态上我把页面局部的短期状态留在组件内，把跨页面共享的用户信息、权限能力、查询条件和字典数据放到公共 store，避免所有状态都堆到全局。权限上除了路由守卫，还在按钮和操作层做 capability 判断，避免只靠前端隐藏。组件复用方面，我把表格搜索、批量操作和审核弹窗抽成业务组件，但保留页面层的组装逻辑，防止抽象过度。后续为了降低维护成本，我还补齐了模块约定文档、关键流程的 E2E 用例和发布前检查表。",
  ...
  },
  {
    ...
    "referenceAnswer": "（示例）报名下单和支付的全链路功能。用户在前端填写信息后，后端会创建预订单，支付成功后再把状态流转到正式订单。这里的难点是用户体验希望操作立刻有反馈，但支付和通知又是异步的。前端侧采用状态机式交互，让每个阶段都有明确可见的等待、成功和失败状态；后端则通过幂等 key、回调验签和延时补偿避免重复下单或状态丢失。前后端之间我们先定义好状态值、错误码和提示语，减少各自理解不一致带来的扯皮。最终用户完成率提升了，而且异常订单的人工处理量明显下降。",
  ...
  },
  {
    ...
    "referenceAnswer": "我做过一条支付成功后生成正式订单并扣减库存的链路。业务上要求用户感知尽量快，但支付回调、库存服务和下游通知又都是异步的。职责包括订单状态流转和一致性治理，采用的方案是先落订单事件，再通过消息队列驱动库存和通知。为了防止重复消费，我在消费者侧用业务单号加事件类型做幂等表，并把状态机设计成只能按预期方向流转。支付回调可能重复或乱序，所以我把回调验签、状态校验和去重放在入口层处理；如果库存服务超时，就记录补偿任务并按退避策略重试，而不是在主链路里无限阻塞。之所以没有上强一致事务，是因为链路跨服务且峰值流量高，优先保证下单成功率和可恢复性。上线后异常订单需要人工介入的比例明显下降，峰值期间链路也更稳。",
  ...
  },
  {
    ...
    "referenceAnswer": "（示例）运营活动配置后台，页面里有复杂筛选、大表格、批量编辑和权限控制。最开始的问题是筛选项一多以后页面渲染会卡，用户切换 tab 还容易丢状态。职责包括页面架构和核心交互，先把瞬时 UI 状态、本地编辑态和跨页面共享状态分层，避免所有东西都塞进全局 store。性能上我先用 Performance 和 React DevTools 找到瓶颈，发现主要是大列表重复渲染和筛选条件变化触发了过多无效更新，所以后面做了虚拟滚动、列表区块拆分和请求结果缓存。为了让交互一致，我把批量操作、保存中、保存失败和回滚提示都统一到状态机里，而不是每个组件各自判断。多人协作上我抽了表格查询框架和权限指令，但保留业务页面自己的组装层，避免抽象过度。这样做的代价是前期约束更多，但后面新页面接入速度和线上稳定性都好了很多。",
  ...
  },
  {
    ...
    "referenceAnswer": "（示例）基于 Python 的异步任务平台，负责处理文件导入、数据清洗和结果回写。业务特点是任务耗时差异很大，而且下游依赖不稳定，所以不应将所有逻辑都放在同步接口里，而是把请求拆成提交、排队、执行和回写几个阶段。职责包括任务模型、worker 执行框架和失败治理。任务定义里会带超时、重试策略和幂等键；执行层按任务类型分队列，避免大任务把小任务全部堵住。失败后不是一律重试，而是区分临时故障和脏数据，临时故障按退避策略重试，数据问题直接落到人工处理队列。可观测性上我补了任务成功率、排队时长、重试次数和死信量指标，并在日志里串联 trace id 方便回放。这样做的取舍是系统复杂度更高，但换来了吞吐稳定性和更低的人工排障成本。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先将现象拆开，确认是接口慢带着 worker 积压，还是 worker 本身打满 CPU 反过来拖慢服务。先看请求 RT、队列长度、worker 并发数和 CPU 使用率的时间线，判断问题从哪一层开始。接着用 profile 或采样看热点函数，如果 CPU 主要耗在 JSON 序列化、压缩、数据处理这类纯 Python 代码，就要考虑 GIL 和 CPU 密集任务不适合堆线程；如果大量时间卡在数据库、网络或对象存储调用，就更像 I/O 阻塞或下游抖动。再往下看任务模型是不是把长任务和短任务混在一起、重试是否放大了积压、序列化 payload 是否过大。最后再决定是拆队列、改进程模型、把 CPU 密集逻辑下沉到独立 worker，还是先限流止血。",
  ...
  },
  {
    ...
    "referenceAnswer": "（示例） Go 的异步导出和回调服务，特点是高峰期会同时收到很多导出请求，而且下游回调偶尔会变慢。职责包括 worker 执行框架和流量保护。并发模型上，不应简单地每个请求起一串 goroutine，而是拆成接入层、任务队列和执行 worker 三层：接入层负责快速校验和入队，执行层按任务类型分 worker 池，避免大任务拖死小任务。goroutine 之间通过 channel 传递任务，但所有下游调用都带 context timeout 和 cancel，确保请求超时后不会把后台协程一直挂住。为了避免堆积扩散，我加了队列长度阈值、并发上限和熔断降级，先保住核心任务，低优先级导出允许延后。这样设计的取舍是实现复杂一些，但比无限起协程更可控，线上延迟抖动和 goroutine 泄漏风险都更低。",
  ...
  },
  {
    ...
    "referenceAnswer": "用 map + 双向链表。map 的 key 指向链表节点，这样 get 时可以 O(1) 找到节点；链表负责维护最近使用顺序，头部放最新访问的节点，尾部放最久没用的节点。get(key) 如果不存在返回空；如果存在，就把这个节点从原位置摘下来移动到头部。put(key, value) 如果 key 已存在，更新 value 并移动到头部；如果不存在，新建节点插到头部，同时写入 map。超过容量时，删除尾部节点，并把尾部 key 从 map 里删掉。这里必须用双向链表，因为移动和删除中间节点需要 O(1) 处理前后指针。",
  ...
  },
  {
    ...
    "referenceAnswer": "用栈保存还没有匹配的左括号。遍历每个字符时，如果是 \"(\"、\"[\"、\"{\" 就压栈；如果是右括号，就先看栈是否为空，空的话直接返回 false，因为右括号出现在了没有左括号的位置。栈不为空时弹出栈顶，用映射关系判断是否和当前右括号成对，例如 \")\" 必须匹配 \"(\"。遍历结束后还要检查栈是否为空，只有为空才说明所有左括号都闭合了。这个算法每个字符最多入栈出栈一次，所以时间 O(n)，最坏情况下栈保存所有左括号，空间 O(n)。",
  ...
  },
  {
    ...
    "referenceAnswer": "返回一个新函数，并在外层闭包里保存 timer。每次触发新函数时，先清掉上一次 timer，再创建新的 setTimeout。等 wait 时间内没有再次触发时，才用 fn.apply(this, args) 执行原函数，这样可以保留调用方的 this 和参数。基础代码大概是：let timer; return function(...args) { clearTimeout(timer); timer = setTimeout(() => fn.apply(this, args), wait); }。如果是搜索框联想或窗口 resize，这种方式可以减少高频触发带来的重复请求或重复计算。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先写递归版：准备一个 result，遍历输入数组，如果 Array.isArray(item) 为 true，就递归处理 item，并把里面的元素继续 push 到 result；否则直接 push 当前元素。伪代码是：function flatten(arr) { const res = []; for (const item of arr) { if (Array.isArray(item)) res.push(...flatten(item)); else res.push(item); } return res; }。复杂度上，每个元素访问一次，所以时间 O(n)，额外空间包括结果数组和递归栈。如果数组嵌套特别深，改成显式栈迭代，避免调用栈溢出。",
  ...
  },
  {
    ...
    "referenceAnswer": "按队列变化来分析。第一行 console.log(\"A\") 是同步代码，立即输出 A。setTimeout 注册回调，放到后续宏任务里。Promise.resolve().then(...) 会把第一个 then 回调放入微任务队列，但不会立刻执行。最后一行 console.log(\"E\") 也是同步代码，所以第二个输出是 E。同步栈清空后，事件循环先处理微任务：执行第一个 then 输出 C；这个 then 执行完成后，链式的第二个 then 才进入微任务队列并继续执行，输出 D。微任务队列清空后，才轮到宏任务里的 setTimeout，输出 B。因此顺序是 A、E、C、D、B。回答时我还会提醒：浏览器和 Node 的事件循环细节有差异，但在这类基础 Promise 与 setTimeout 输出题里，核心判断是同步优先、微任务优先于宏任务。",
    ...
    "scoringRubric": {
      ...
      "redFlags": [
        "把 Promise.then 说成同步执行",
        "把 setTimeout 说成一定最先执行",
        "只给答案不讲调度过程"
      ]
    }
  },
  ...
  },
  {
    ...
    "referenceAnswer": "首先明确限流维度，比如用户、IP、接口或租户。最简单可以用 Redis 固定窗口计数器：请求进来后拼 key，例如 rate:userId:api:yyyyMMddHHmm，对 key 执行 INCR，第一次创建时设置 60 秒过期；如果计数超过阈值，直接返回 429 或降级提示。这个方案实现简单，但窗口边界可能在两分钟交界处放过双倍流量，所以高要求场景换成滑动窗口或令牌桶。分布式场景下，INCR 和 EXPIRE 最好用 Lua 合并，避免设置过期失败导致 key 长期存在。",
  ...
  },
  {
    ...
    "referenceAnswer": "把流程拆成三步。第一步先查缓存，命中正常返回；第二步如果缓存未命中，先用布隆过滤器判断 key 是否可能存在，如果一定不存在就直接拒绝或返回空结果；第三步如果可能存在再回源数据库。数据库查不到时，不是直接什么都不做，而是写入一个短 TTL 的空值缓存，比如 1 到 5 分钟，避免同一个不存在 key 反复打 DB。边界上要控制空值 TTL，数据写入时要删除对应空值缓存；如果异常流量很大，还要对回源加限流、热点 key 监控和降级策略。",
  ...
  },
  {
    ...
    "referenceAnswer": "首先区分幂等键的粒度：下单接口可以用 clientRequestId 加用户维度，支付回调用支付流水号，MQ 消费可以用业务单号加事件类型。服务端收到请求后先尝试插入幂等记录，状态是处理中；如果唯一键冲突，就查询已有记录，成功则直接返回历史结果，处理中则返回处理中或做短暂等待，失败则看是否允许重试。业务处理和幂等状态更新最好在同一个本地事务里完成：先校验状态机是否允许流转，再写业务数据，最后把幂等记录改成成功并保存返回摘要。高并发下不能只靠先查再写，要用唯一索引、行锁或原子插入兜底；异常时要有超时恢复任务，把长期处理中记录标记为可重试或人工介入。这样重复请求、重试、回调乱序和 MQ 重投都能被收敛到同一个业务结果上。幂等：先插处理中记录+唯一索引兜底，成功则返回历史结果；支付回调与 MQ 消费都要业务单号去重。",
  ...
  },
  {
    ...
    "type": "knowledge",
    "difficulty": 2,
    "question": "如果让你为一个下单接口设计测试方案，你会怎么拆功能测试、接口测试、异常场景和回归策略？",
    "keywords": [
      "测试范围",
      "正常流程",
      "异常场景",
    ...
    "referenceAnswer": "首先按风险拆测试范围。功能层先覆盖正常下单、库存不足、优惠券失效、地址错误、重复提交等主流程和异常流程；接口层检查参数校验、状态码、返回字段、幂等键、超时重试和数据库状态变化；如果依赖库存和支付服务，还会做 mock 或联调验证。边界上补用户并发提交、同一订单重复回调、网络抖动重试、数据库回滚失败等场景。回归策略上，先沉淀核心链路自动化用例作为冒烟，再维护稳定的接口自动化回归集，发布前结合变更范围做针对性回归。",
    "excellentAnswer": "我会先按风险拆测试范围。功能层先覆盖正常下单、库存不足、优惠券失效、地址错误、重复提交等主流程和异常流程；接口层检查参数校验、状态码、返回字段、幂等键、超时重试和数据库状态变化；如果依赖库存和支付服务，还会做 mock 或联调验证。边界上我会补用户并发提交、同一订单重复回调、网络抖动重试、数据库回滚失败等场景。回归策略上，先沉淀核心链路自动化用例作为冒烟，再维护稳定的接口自动化回归集，发布前结合变更范围做针对性回归。",
    "followUps": [
      "你怎么判断哪些场景必须自动化，哪些适合手工验证？",
      "如果这个接口涉及库存扣减和支付回调，你会补哪些一致性测试？",
      "测试方案里如何体现优先级和风险排序？"
    ],
    "scoringRubric": {
      "mustHave": [
  ...
  },
  {
    ...
    "type": "project",
    "difficulty": 3,
    "question": "请讲一个你主导测试质量提升或自动化建设的项目，重点说明问题现状、你的方案、落地阻力和结果。",
    "keywords": [
      "现状问题",
      "自动化方案",
      "质量门禁",
    ...
    "referenceAnswer": "我做过一次接口自动化和发布门禁建设。原来每次发版主要靠手工回归，核心链路回归要半天，漏测导致线上回滚也比较频繁。职责包括梳理核心接口清单，基于接口契约和测试数据分层建设自动化用例，把下单、支付、退款等高风险链路纳入冒烟门禁，同时把失败用例和最近变更模块关联到发布流程里。推进中最大的阻力是历史接口不稳定、测试环境数据脏和研发担心接入门禁影响发布效率，通过先做高收益的核心链路、补测试桩和失败重试机制来降低阻力。最终核心回归从半天缩到 30 分钟内，关键链路漏测率和发布回滚次数都明显下降。",
    "excellentAnswer": "我做过一次接口自动化和发布门禁建设。原来每次发版主要靠手工回归，核心链路回归要半天，漏测导致线上回滚也比较频繁。我负责梳理核心接口清单，基于接口契约和测试数据分层建设自动化用例，把下单、支付、退款等高风险链路纳入冒烟门禁，同时把失败用例和最近变更模块关联到发布流程里。推进中最大的阻力是历史接口不稳定、测试环境数据脏和研发担心接入门禁影响发布效率，我通过先做高收益的核心链路、补测试桩和失败重试机制来降低阻力。最终核心回归从半天缩到 30 分钟内，关键链路漏测率和发布回滚次数都明显下降。",
    "followUps": [
      "自动化用例不稳定时你怎么治理，而不是简单跳过？",
      "质量门禁卡住发布时，你怎么和研发一起判断是放行还是修复？",
      "你如何衡量自动化建设是否真的产生价值？"
    ],
    "scoringRubric": {
      "mustHave": [
  ...
  },
  {
    ...
    "type": "knowledge",
    "difficulty": 2,
    "question": "一台 Linux 服务器突然 CPU 飙高、接口超时，你会按什么顺序排查？请说明你会看哪些系统和业务信号。",
    "keywords": [
      "CPU",
      "负载",
      "进程线程",
    ...
    "referenceAnswer": "首先确认影响范围，是单机异常还是集群普遍异常，以及从什么时候开始。系统层先看 CPU 使用率和 load，区分用户态、内核态、iowait，再看 top、pidstat 找到高占用进程和线程；如果磁盘或网络异常，也会检查 iostat、sar、ss 这类指标。随后看系统日志和最近变更，确认有没有发布、定时任务、备份脚本或流量突增。业务层再对齐应用日志、网关超时、下游依赖和错误码，判断是代码热点、GC、慢 SQL、连接池耗尽还是网络抖动。排查顺序一定是先止血，再定位根因，最后补监控和预案。",
    "excellentAnswer": "我会先确认影响范围，是单机异常还是集群普遍异常，以及从什么时候开始。系统层先看 CPU 使用率和 load，区分用户态、内核态、iowait，再看 top、pidstat 找到高占用进程和线程；如果磁盘或网络异常，也会检查 iostat、sar、ss 这类指标。随后看系统日志和最近变更，确认有没有发布、定时任务、备份脚本或流量突增。业务层再对齐应用日志、网关超时、下游依赖和错误码，判断是代码热点、GC、慢 SQL、连接池耗尽还是网络抖动。排查顺序一定是先止血，再定位根因，最后补监控和预案。",
    "followUps": [
      "load 很高但 CPU 使用率不高时你会怀疑什么？",
      "如果只有一台机器异常，你如何快速止血？",
      "排查完成后你会补哪些监控和预案？"
    ],
    "scoringRubric": {
  ...
  },
  {
    ...
    "type": "project",
    "difficulty": 3,
    "question": "请讲一个你处理过的线上故障或重大稳定性问题，重点说清告警发现、排查链路、止血动作和后续治理。",
    "keywords": [
      "告警发现",
      "排查链路",
      "止血动作",
    ...
    "referenceAnswer": "（示例）数据库连接数被打满导致核心接口大面积超时的故障。最初是监控平台告警和业务侧投诉同时出现，我先确认影响范围集中在订单服务和数据库实例，然后通过应用日志、连接池指标和数据库状态发现连接泄漏叠加流量突增。止血上先做读流量降级、临时扩容连接池上限并限制非核心任务，确保主链路恢复。随后联合研发定位到某个批处理任务没有正确释放连接，导致高峰期连接堆积。事后我补了连接池使用率告警、慢 SQL 和连接泄漏监控，也把高风险任务改到低峰执行并加入压测校验。",
    "excellentAnswer": "我处理过一次数据库连接数被打满导致核心接口大面积超时的故障。最初是监控平台告警和业务侧投诉同时出现，我先确认影响范围集中在订单服务和数据库实例，然后通过应用日志、连接池指标和数据库状态发现连接泄漏叠加流量突增。止血上先做读流量降级、临时扩容连接池上限并限制非核心任务，确保主链路恢复。随后联合研发定位到某个批处理任务没有正确释放连接，导致高峰期连接堆积。事后我补了连接池使用率告警、慢 SQL 和连接泄漏监控，也把高风险任务改到低峰执行并加入压测校验。",
    "followUps": [
      "为什么你的止血动作是这些，而不是直接扩容数据库？",
      "如何避免故障处理只停留在“人肉救火”？",
      "故障复盘里你最关注哪些可执行改进项？"
    ],
    "scoringRubric": {
  ...
  },
  {
    ...
    "type": "knowledge",
    "difficulty": 2,
    "question": "你会如何设计一个从代码提交到生产发布的 CI/CD 流水线？请说明构建、测试、发布、回滚和权限控制。",
    "keywords": [
      "CI/CD",
      "构建",
      "测试",
    ...
    "referenceAnswer": "把流水线拆成几个明确阶段。代码提交后先做静态检查和单元测试，再构建制品并上传到统一制品库，保证测试、预发和生产使用同一份产物。进入测试或预发环境时，增加接口冒烟和关键链路校验；生产发布采用灰度或分批发布，并要求审批和发布记录可追踪。回滚上不能只说重新发版，而是要保留上一版本制品和配置快照，必要时一键回滚。权限上区分开发、测试、运维和发布审批角色，避免任何人都能直接推生产。",
    "excellentAnswer": "我会把流水线拆成几个明确阶段。代码提交后先做静态检查和单元测试，再构建制品并上传到统一制品库，保证测试、预发和生产使用同一份产物。进入测试或预发环境时，增加接口冒烟和关键链路校验；生产发布采用灰度或分批发布，并要求审批和发布记录可追踪。回滚上不能只说重新发版，而是要保留上一版本制品和配置快照，必要时一键回滚。权限上我会区分开发、测试、运维和发布审批角色，避免任何人都能直接推生产。",
    "followUps": [
      "为什么强调测试到生产用同一份制品？",
      "灰度发布失败后，你怎么判断继续、暂停还是回滚？",
      "流水线很慢时，你会优先优化哪几段？"
    ],
    "scoringRubric": {
      "mustHave": [
  ...
  },
  {
    ...
    "type": "system-design",
    "difficulty": 3,
    "question": "如果让你建设一套面向业务团队的可观测性平台，你会怎么设计日志、指标、链路追踪和告警降噪？",
    "keywords": [
      "日志",
      "指标",
      "链路追踪",
    ...
    "referenceAnswer": "首先明确目标不是“把数据收上来”，而是提高定位效率和稳定性治理能力。指标层收集主机、容器、应用和业务指标，并围绕延迟、错误率、吞吐量设计 SLI/SLO；日志层统一结构化字段，例如 traceId、服务名、实例、环境和错误码，保证能跨系统检索；链路追踪用于还原请求路径和慢点。告警不能简单按阈值堆数量，做分级、聚合、抑制和维护窗口，避免告警风暴。平台层面还要考虑采集成本、存储保留策略、多租户权限，以及给业务团队提供标准仪表盘和排障入口。",
    "excellentAnswer": "我会先明确目标不是“把数据收上来”，而是提高定位效率和稳定性治理能力。指标层收集主机、容器、应用和业务指标，并围绕延迟、错误率、吞吐量设计 SLI/SLO；日志层统一结构化字段，例如 traceId、服务名、实例、环境和错误码，保证能跨系统检索；链路追踪用于还原请求路径和慢点。告警不能简单按阈值堆数量，我会做分级、聚合、抑制和维护窗口，避免告警风暴。平台层面还要考虑采集成本、存储保留策略、多租户权限，以及给业务团队提供标准仪表盘和排障入口。",
    "followUps": [
      "为什么很多团队有监控数据却定位效率仍然很差？",
      "告警降噪和漏报之间怎么平衡？",
      "日志、指标、trace 之间你会如何建立关联？"
    ],
    "scoringRubric": {
      "mustHave": [
        "日志",
  ...
  },
  {
    ...
    "type": "knowledge",
    "difficulty": 2,
    "question": "请说明你理解的数据仓库分层思路，以及 ODS、DWD、DWS、ADS 各层通常解决什么问题。",
    "keywords": [
      "ODS",
      "DWD",
      "DWS",
    ...
    "referenceAnswer": "把数仓分层理解为“稳定接入、统一建模、复用指标、面向应用”。ODS 主要承接源系统原始数据，重点是保留原貌和可追溯；DWD 负责清洗、去重、补全维度并形成可复用的业务明细；DWS 在主题域上做公共聚合，例如用户、订单、交易等核心指标沉淀；ADS 则面向具体报表、运营分析或接口服务输出。分层的价值不是名字本身，而是避免每个报表都各自写逻辑，导致口径混乱和维护成本失控。",
    "excellentAnswer": "我会把数仓分层理解为“稳定接入、统一建模、复用指标、面向应用”。ODS 主要承接源系统原始数据，重点是保留原貌和可追溯；DWD 负责清洗、去重、补全维度并形成可复用的业务明细；DWS 在主题域上做公共聚合，例如用户、订单、交易等核心指标沉淀；ADS 则面向具体报表、运营分析或接口服务输出。分层的价值不是名字本身，而是避免每个报表都各自写逻辑，导致口径混乱和维护成本失控。",
    "followUps": [
      "如果业务部门和报表口径经常不一致，你会怎么治理？",
      "哪些逻辑适合放 DWD，哪些更适合放 DWS？",
      "分层过多会带来什么成本？"
    ],
    "scoringRubric": {
      "mustHave": [
  ...
  },
  {
    ...
    "type": "project",
    "difficulty": 3,
    "question": "请讲一个你做过的数据链路或数仓建设项目，重点说明数据来源、调度依赖、质量保障和业务价值。",
    "keywords": [
      "数据来源",
      "调度依赖",
      "数据质量",
    ...
    "referenceAnswer": "（示例）经营分析数仓建设，数据来源包括订单库、支付库、用户行为日志和 CRM。我的工作是梳理主题域和核心指标口径，把原本分散在各报表里的逻辑沉淀到 DWD/DWS，并用调度平台管理依赖关系和失败重跑。质量上，我补了主键去重、空值校验、分区完整性检查、核心指标波动告警和补数流程，避免上游晚到数据导致报表失真。业务价值上，经营日报和活动复盘统一了口径，报表开发效率提升，业务部门对数据争议明显减少。",
    "excellentAnswer": "我参与过一次经营分析数仓建设，数据来源包括订单库、支付库、用户行为日志和 CRM。我的工作是梳理主题域和核心指标口径，把原本分散在各报表里的逻辑沉淀到 DWD/DWS，并用调度平台管理依赖关系和失败重跑。质量上，我补了主键去重、空值校验、分区完整性检查、核心指标波动告警和补数流程，避免上游晚到数据导致报表失真。业务价值上，经营日报和活动复盘统一了口径，报表开发效率提升，业务部门对数据争议明显减少。",
    "followUps": [
      "如果上游晚到或数据回刷，你怎么处理补数和口径一致？",
      "数据质量告警很多但没人处理，你会怎么治理？",
      "这个项目里你做的最关键技术决策是什么？"
    ],
    "scoringRubric": {
      "mustHave": [
  ...
  },
  {
    ...
    "type": "knowledge",
    "difficulty": 2,
    "question": "如果让你评估一个分类模型或推荐模型的效果，你会看哪些指标？离线评估和线上评估有什么区别？",
    "keywords": [
      "离线评估",
      "线上评估",
      "准确率",
    ...
    "referenceAnswer": "首先看任务目标，再选指标。分类任务常见是 precision、recall、F1、AUC，不同业务对误报和漏报容忍度不同；推荐或排序还会关注点击率、转化率、NDCG、留存等业务指标。离线评估主要解决模型之间的相对比较和调参效率问题，但它受训练数据分布和标注质量影响，不能完全代表真实线上效果。线上评估则要通过 A/B 实验看真实用户行为变化，同时关注推理延迟、资源成本和稳定性，因为一个离线指标更好的模型未必值得上线。",
    "excellentAnswer": "我会先看任务目标，再选指标。分类任务常见是 precision、recall、F1、AUC，不同业务对误报和漏报容忍度不同；推荐或排序还会关注点击率、转化率、NDCG、留存等业务指标。离线评估主要解决模型之间的相对比较和调参效率问题，但它受训练数据分布和标注质量影响，不能完全代表真实线上效果。线上评估则要通过 A/B 实验看真实用户行为变化，同时关注推理延迟、资源成本和稳定性，因为一个离线指标更好的模型未必值得上线。",
    "followUps": [
      "为什么离线指标提升不一定带来线上收益？",
      "精确率和召回率冲突时，你如何取舍？",
      "如果模型效果退化，你会先怀疑数据还是模型？为什么？"
    ],
    "scoringRubric": {
      "mustHave": [
        "离线评估",
  ...
  },
  {
    ...
    "type": "project",
    "difficulty": 3,
    "question": "请讲一个你做过的模型落地或大模型应用项目，重点说明数据、效果评估、上线方式和成本取舍。",
    "keywords": [
      "数据来源",
      "效果评估",
      "上线方式",
    ...
    "referenceAnswer": "（示例）基于检索增强生成的知识问答项目。数据侧先清洗知识库文档，做切片、去重和元数据补充，再建立向量索引；效果评估上不仅看人工标注问答集的命中率和答案相关性，还会在线上看提问成功率、人工转接率和响应时延。上线时我把检索、重排和生成拆开监控，先灰度给内部团队使用，再逐步放量。成本上没有盲目追求大模型参数规模，而是通过缓存高频问答、控制上下文长度和区分模型档位，把效果和推理成本控制在可接受范围内。",
    "excellentAnswer": "我做过一个基于检索增强生成的知识问答项目。数据侧先清洗知识库文档，做切片、去重和元数据补充，再建立向量索引；效果评估上不仅看人工标注问答集的命中率和答案相关性，还会在线上看提问成功率、人工转接率和响应时延。上线时我把检索、重排和生成拆开监控，先灰度给内部团队使用，再逐步放量。成本上没有盲目追求大模型参数规模，而是通过缓存高频问答、控制上下文长度和区分模型档位，把效果和推理成本控制在可接受范围内。",
    "followUps": [
      "如果用户反馈答案“看起来合理但其实错了”，你会怎么治理？",
      "为什么你选择当前部署方式，而不是全部托管给外部平台？",
      "模型效果、时延和成本冲突时你怎么做取舍？"
    ],
    "scoringRubric": {
      "mustHave": [
  ...
  },
  {
    ...
    "type": "knowledge",
    "difficulty": 2,
    "question": "请解释 XSS、CSRF 和 SQL 注入分别是什么，有哪些常见防护手段？",
    "keywords": [
      "XSS",
      "CSRF",
      "SQL 注入",
    ...
    "referenceAnswer": "这三类问题要区分攻击面。XSS 发生在浏览器端，核心是不要把不可信内容原样输出到 HTML、JS 或 URL 上，所以要做上下文相关的输出编码，并辅以 CSP。CSRF 的前提是用户已经登录且浏览器会自动带上凭证，所以要用 CSRF token、SameSite Cookie，并对转账这类高风险操作做二次确认。SQL 注入发生在服务端查询层，关键防护不是简单过滤特殊字符，而是参数化查询、ORM 正确使用和数据库最小权限。回答时我也会强调，安全问题不能只靠单点修补，要有编码规范、扫描和验证闭环。",
    "excellentAnswer": "这三类问题要区分攻击面。XSS 发生在浏览器端，核心是不要把不可信内容原样输出到 HTML、JS 或 URL 上，所以要做上下文相关的输出编码，并辅以 CSP。CSRF 的前提是用户已经登录且浏览器会自动带上凭证，所以要用 CSRF token、SameSite Cookie，并对转账这类高风险操作做二次确认。SQL 注入发生在服务端查询层，关键防护不是简单过滤特殊字符，而是参数化查询、ORM 正确使用和数据库最小权限。回答时我也会强调，安全问题不能只靠单点修补，要有编码规范、扫描和验证闭环。",
    "followUps": [
      "为什么 SQL 注入的核心防护是参数化，而不是黑名单过滤？",
      "SameSite Cookie 对 CSRF 防护起什么作用？",
      "如果站点允许用户发布富文本，怎样降低 XSS 风险？"
    ],
    "scoringRubric": {
      "mustHave": [
        "XSS",
  ...
  },
  {
    ...
    "type": "project",
    "difficulty": 3,
    "question": "请讲一个你处理过的安全漏洞或安全治理项目，重点说明风险评估、修复方案、验证方法和后续防复发措施。",
    "keywords": [
      "风险评估",
      "修复方案",
      "验证方法",
    ...
    "referenceAnswer": "（示例）后台接口越权问题。问题最初来自安全测试，发现普通用户通过修改资源 ID 可以访问不属于自己的数据。我先评估影响范围，确认是对象级授权校验缺失而不是单个接口 bug，于是推动研发把权限校验下沉到统一服务层，而不是每个接口各自判断。修复后我除了做回归验证和安全复测，还补了权限相关单测、接口扫描规则和代码审查清单，避免后续新增接口继续漏掉对象级鉴权。这个项目里重点不是补一个 if 判断，而是建立统一权限模型和验证闭环。",
    "excellentAnswer": "我处理过一次后台接口越权问题。问题最初来自安全测试，发现普通用户通过修改资源 ID 可以访问不属于自己的数据。我先评估影响范围，确认是对象级授权校验缺失而不是单个接口 bug，于是推动研发把权限校验下沉到统一服务层，而不是每个接口各自判断。修复后我除了做回归验证和安全复测，还补了权限相关单测、接口扫描规则和代码审查清单，避免后续新增接口继续漏掉对象级鉴权。这个项目里重点不是补一个 if 判断，而是建立统一权限模型和验证闭环。",
    "followUps": [
      "为什么很多权限问题修完一个接口后还会反复出现？",
      "安全修复和业务上线冲突时，你怎么做风险权衡？",
      "你如何验证修复没有引入新的业务回归？"
    ],
    "scoringRubric": {
      "mustHave": [
  ...
  },
  {
    ...
    "type": "knowledge",
    "difficulty": 3,
    "question": "如果一个核心业务系统要从单体逐步演进到更高可用、更易扩展的架构，你会怎么规划演进路径，而不是一次性推倒重来？",
    "keywords": [
      "演进路径",
      "高可用",
      "拆分边界",
    ...
    "referenceAnswer": "不应一上来就建议全面微服务化，而是先做现状诊断：性能瓶颈在哪里，稳定性问题集中在哪，团队是否有足够的治理能力。演进上通常先补观测能力、发布回滚、数据库和缓存治理，把现有系统的可见性和稳定性提起来；随后再围绕高变更、高压力或边界清晰的模块做逐步抽离，比如订单、库存、消息通知，而不是按组织图拆。每一步都要有收益指标和回退方案，避免系统复杂度先上去了，业务收益却没有跟上。架构演进本质上是持续治理，而不是一次性重构。",
    "excellentAnswer": "我不会一上来就建议全面微服务化，而是先做现状诊断：性能瓶颈在哪里，稳定性问题集中在哪，团队是否有足够的治理能力。演进上通常先补观测能力、发布回滚、数据库和缓存治理，把现有系统的可见性和稳定性提起来；随后再围绕高变更、高压力或边界清晰的模块做逐步抽离，比如订单、库存、消息通知，而不是按组织图拆。每一步都要有收益指标和回退方案，避免系统复杂度先上去了，业务收益却没有跟上。架构演进本质上是持续治理，而不是一次性重构。",
    "followUps": [
      "为什么很多“架构升级”最后反而让交付效率下降？",
      "你如何判断一个模块适不适合先拆？",
      "演进过程中如何控制跨团队协作成本？"
    ],
    "scoringRubric": {
  ...
  },
  {
    ...
    "type": "project",
    "difficulty": 3,
    "question": "请讲一个你主导架构治理、技术决策或团队研发流程改进的项目，重点说清背景、决策依据、推进阻力和结果。",
    "keywords": [
      "背景",
      "决策依据",
      "推进阻力",
    ...
    "referenceAnswer": "（示例）核心交易系统的技术治理。背景是系统功能长期堆叠，发布风险高、问题定位慢，团队每次上线都很被动。我先用故障数据、发布回滚记录和研发效率指标证明问题不只是“代码旧”，而是缺少统一边界、发布规范和可观测性。治理动作上，不应直接推动大重构，而是先统一日志和告警标准、建立发布检查清单、梳理高风险模块边界，再逐步推进缓存治理和异步解耦。推进中最大的阻力是业务排期和团队对治理收益不直观，通过阶段性指标和几个故障案例把优先级拉齐。最终发布成功率提升，故障定位时间下降，团队对系统边界也更清楚。",
    "excellentAnswer": "我主导过一次核心交易系统的技术治理。背景是系统功能长期堆叠，发布风险高、问题定位慢，团队每次上线都很被动。我先用故障数据、发布回滚记录和研发效率指标证明问题不只是“代码旧”，而是缺少统一边界、发布规范和可观测性。治理动作上，我没有直接推动大重构，而是先统一日志和告警标准、建立发布检查清单、梳理高风险模块边界，再逐步推进缓存治理和异步解耦。推进中最大的阻力是业务排期和团队对治理收益不直观，我通过阶段性指标和几个故障案例把优先级拉齐。最终发布成功率提升，故障定位时间下降，团队对系统边界也更清楚。",
    "followUps": [
      "面对业务压力时，你如何证明治理值得投入？",
      "技术决策出现分歧时，你如何推进而不是拍脑袋定方案？",
      "如果治理投入了一段时间却没有明显收益，你会怎么调整？"
    ],
    "scoringRubric": {
      "mustHave": [
        "背景",
